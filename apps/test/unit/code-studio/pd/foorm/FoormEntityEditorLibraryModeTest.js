...
describe('FoormEntityEditor in Library editing mode', () => {
  ...
  it('can save new library question in existing library', () => {
    ...
    expect(_.some(fetchableLibraryQuestions, ['id', 1])).toBe(false);
    ...
    expect(_.some(fetchableLibraryQuestions, ['id', 1])).toBeTruthy();
  });
  ...
  it('can save new library question in new library', () => {
    ...
    expect(_.some(fetchableLibraries, ['id', 2])).toBe(false);
    expect(_.some(fetchableLibraryQuestions, ['id', 1])).toBe(false);
    ...
    expect(_.some(fetchableLibraries, ['id', 2])).toBeTruthy();
    expect(_.some(fetchableLibraryQuestions, ['id', 1])).toBeTruthy();
  });
